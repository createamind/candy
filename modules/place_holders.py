...
class Place_Holders(object):
    def __init__(self, args):
        ...
        self.control = tf.placeholder(tf.int32, shape=(args.batch_size, 1))

        self.reward = tf.placeholder(tf.float32, shape=(args.batch_size, 1))

        self.next_sequence = tf.placeholder(tf.float32, shape=(args.batch_size, 16, 112, 112, 3))

    def inference(self):

        return [self.image_sequence, self.raw_image, self.depth_image, self.seg_image, self.speed, self.collision, self.intersection, self.control, self.reward, self.next_sequence]
